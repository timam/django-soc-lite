    ...
    def get_method(self):
        ...
        if query:
            ...
            if rule_checker.rfe_filter(str(value)):                         #check attack 
                #logging.info(log(event= "rfe attempt", url= self.request.path, stacktrace= traceback.format_stack(), query_string= str(parameter+'='+quote(value))))
    ...
    def post_method(self):
        ...
        for i in range(len(l)):
            ...
            if rule_checker.rfe_filter(str(value)): 
                #logging.info(log(event= "rfi attempt", url= self.request.path, stacktrace= traceback.format_stack(), query_string= str(parameter+'='+quote(value))))
                q = bleach.clean(value)
                if not isinstance(q, str):
                    q = q.encode("utf-8")
                q = HTML_Escape.CommandEscape(q)
