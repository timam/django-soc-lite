...
def checker(packages):
    ...
    with open(os.path.join(BASE, 'logger.txt'), 'r') as package_last:
...
def update_latest(packages):
    """update latest log list by updated one"""
    with open(os.path.join(BASE, 'logger.txt'), 'w') as package_last:
...
def log_handler():
    ...
    if interval <= 720 or os.path.getsize(os.path.join(BASE, 'logger.txt')) == 0: #less than 12 hours or file is empty
        import pip
